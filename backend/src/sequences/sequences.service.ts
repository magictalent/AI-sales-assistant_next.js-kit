...
export class SequencesService {
  ...
  async startSequence({ clientId, leadId, leadEmail, leadPhone, campaignId, channel = 'email', tone = 'friendly' }: StartSequenceDto) {
    const now = new Date();
    ...
    try {
      if (leadId) {
        ...
        lead = await this.db.findOne('leads', { phone: leadPhone });
        leadId = lead?.id;
      }
    } catch {}
