...
export class SequencesController {
  ...
  @Post('start')
  async start(
    @Body() body: { clientId: string; campaignId: string; leadId?: string; leadEmail?: string; leadPhone?: string; channel?: 'email' | 'sms' | 'whatsapp'; tone?: 'friendly' | 'professional' | 'casual' },
  ) {
    return this.sequences.startSequence(body);
  }

  @Post('tick')
  async tick(@Query('limit') limit?: string, @Query('force') force?: string) {
    const n = limit ? parseInt(limit, 10) : 10;
    const doForce = force === '1' || force === 'true';
    return this.sequences.tick(n, { force: doForce });
  }
  ...
  async startAll(@Body() body: { clientId: string; campaignId: string; channel?: 'email' | 'sms' | 'whatsapp'; tone?: 'friendly' | 'professional' | 'casual'; limit?: number }) {
    try {
      if (!body?.clientId || !body?.campaignId) {
        throw new Error('Missing clientId or campaignId');
      }
      return await this.sequences.startSequenceForAllLeads({
      ...
      console.error('[SequencesController.start-all] Error:', e?.message || e);
      throw e;
    }
  }
}
