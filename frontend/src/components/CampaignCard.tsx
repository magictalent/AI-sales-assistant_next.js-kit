    ...
    <div className={`relative bg-white rounded-xl shadow p-6 hover:shadow-lg transition-shadow ${highlighted ? 'ring-2 ring-violet-500' : ''}`}>
            ...
            onClick={async () => {
              ...
              try {
                ...
                const res = await fetch(`${API_BASE}/sequences/start-all`, {
                  ...
                  body: JSON.stringify({ clientId: campaign.client_id, campaignId: campaign.id, channel: campaign.channel, tone: (campaign as any).tone || 'friendly' })
                })
                const data = await res.json()
                if (!res.ok) throw new Error(data?.message || 'Failed to enqueue sequences')
                setSeqMsg(`Enqueued ${data.enqueued} leads (${data.steps_created} steps) for '${campaign.name}'. Sending first ${campaign.channel} messages…`)
      ...
      {showSequence && (
        <div className="mt-5 rounded-lg border bg-gray-50 p-4">
          <div className="text-sm text-gray-700 mb-3">Start sequence for a single lead ({campaign.channel})</div>
          <div className="flex gap-2 mb-3 items-center">
            {campaign.channel === 'email' ? (
              ...
              <input value={leadEmail} onChange={(e)=>setLeadEmail(e.target.value)} placeholder="+15551234567" className="flex-1 px-3 py-2 border border-gray-300 rounded-md text-sm placeholder-gray-400" />
            )}
            <button
              onClick={async () => {
                setSeqMsg('')
                if (!token) { setSeqMsg('Please log in'); return }
                if (!leadEmail) { setSeqMsg(campaign.channel === 'email' ? 'Enter a lead email' : 'Enter a lead phone (+E.164)'); return }
                try {
                  setStarting(true)
                  const res = await fetch(`${API_BASE}/sequences/start`, {
                    method: 'POST', headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
                    body: JSON.stringify({ clientId: campaign.client_id, campaignId: campaign.id, ...(campaign.channel === 'email' ? { leadEmail } : { leadPhone: leadEmail }), channel: campaign.channel })
                  })
                  const data = await res.json()
